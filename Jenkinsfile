   ...
   stage('deploy App')
    script {
        ...
        openshift.withProject(namespace) {
                           def aplicacao = openshift.newApp("--name=${env.BRANCH_NAME}-api-${env.BUILD_NUMBER}",  --allow-missing-images ","--context-dir=/", "--source-secret=token-openshift","showBuildLogs=true").expose();
